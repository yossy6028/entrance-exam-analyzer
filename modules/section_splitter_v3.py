...
class ImprovedSectionSplitterV3:
    ...
    def _filter_true_markers(self, candidates: List[Dict], text: str) -> List[Dict]:
        ...
        for candidate in candidates:
            ...
            # 適応的距離チェック（優先度に応じて閾値を調整）
            distance = candidate['start'] - prev_marker['start']
            
            # 優先度に基づく最小距離の動的計算
            min_distance = self._calculate_min_distance(prev_marker['priority'], candidate['priority'])
            ...
            # 番号が戻る場合は必ず拒否（OCRノイズ対策）
            if candidate['number'] <= prev_marker['number']:
                continue
            
            # 番号が大幅に飛ぶ場合（3問以上飛ぶ）も拒否
            if candidate['number'] - prev_marker['number'] > 3:
                # ただし、優先度が非常に高い場合（明確な大問表記）は許容
    ...
    def _is_small_question(self, candidate: Dict, text: str) -> bool:
        ...
        # OCRノイズのパターンを先にチェック（Geminiが指摘した問題）
        ocr_noise_patterns = [
            '解答用紙', '問題用紙', 'ページ', '受験番号', 
            '氏名', '学校名', '合計点', '配点',
            '注意事項', '指示に従って', 'マークシート',
            '億円', '万円', '点満点'  # 数値単位もノイズとして除外
        ]
        
        for noise_pattern in ocr_noise_patterns:
            if noise_pattern in context:
                return True
    ...
    def _calculate_min_distance(self, prev_priority: int, curr_priority: int) -> int:
        """
        マーカー間の最小距離を優先度に基づいて動的に計算
        
        Args:
            prev_priority: 前のマーカーの優先度
            curr_priority: 現在のマーカーの優先度
        ...
        # 高優先度マーカー（明確な大問表記）の場合は距離を緩和
        if curr_priority >= 8:  # 'daimon_explicit' など
            return base_distance // 2  # 50文字
        elif curr_priority >= 6:  # 'bracket_kanji' など
            return base_distance  # 100文字
        else:  # 低優先度（曖昧なパターン）
            return base_distance * 2  # 200文字
