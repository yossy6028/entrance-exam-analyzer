...
class UniversalAnalyzer:
    ...
    def _detect_theme(self, text: str) -> Optional[str]:
        """改善されたテーマ検出 - OCRノイズ除去と参照型問題対応"""
        try:
            # 改善されたテーマ抽出器を使用
            from .improved_theme_extractor import ImprovedThemeExtractor
            
            extractor = ImprovedThemeExtractor()
            result = extractor.analyze_text(text)
            
            # 信頼度が30%以上の場合のみテーマを返す
            if result.confidence >= 30.0:
                return result.theme
            else:
                # フォールバック: 従来の簡易検出
                return self._detect_theme_fallback(text)
    ...
    def _detect_theme_fallback(self, text: str) -> Optional[str]:
        """フォールバック用の簡易テーマ検出"""
        # OCRノイズを簡易除去
        noise_patterns = [
            r'受験番号[：:\s]*\d*', r'採点[欄者][：:\s]*', r'解答用紙',
            r'この人物について?', r'あらわしている?', r'まちがっている文章を',
            r'正しい文章を', r'下線[①②③④⑤⑥⑦⑧⑨⑩]について?'
        ]
        ...
        # 簡易テーマキーワード
        theme_keywords = {
            '友情・人間関係': ['友情', '友達', '友人', '仲間', '一緒', '協力'],
            '家族・親子': ['家族', '父', '母', '親', '子', '家庭'],
            '成長・学び': ['成長', '学び', '体験', '経験', '変化', '発見'],
            '自然・環境': ['自然', '環境', '森', '海', '山', '動物', '植物'],
            '科学・技術': ['科学', '技術', 'AI', 'ロボット', '研究', '発明'],
            '社会・文化': ['社会', '文化', '歴史', '時代', '現代'],
            '哲学・価値観': ['哲学', '思想', '価値観', '生き方', '意味']
        }
        ...
        return detected_theme if max_count >= 2 else '人間と社会'
